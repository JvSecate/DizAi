...
export default function Perfil() {
  ...
  const [formData, setFormData] = useState({
    nome: "", email: "", senha: "", img: "", cnpj: "", setor: "", descricao: ""
  });
    ...
    if (usuario.tipo === "usuario") {
      fetch(`${API_URL}/usuarios/${usuario.id}`)
        ...
        .then(data => setFormData({
          nome: data.nome, email: data.email, senha: "", img: usuarioImg,
          cnpj: "", setor: "", descricao: ""
        }));
    }
        ...
        .then(empresa => {
          fetch(`${API_URL}/usuarios/${usuario.id}`)
            ...
            .then(user => setFormData({
              ...
              img: empresaImg,
              cnpj: empresa.cnpj, setor: empresa.setor, descricao: empresa.descricao
            }));
        });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/feedbacks`)
      .then(res => res.json())
      .then(data => {
        if (tipo === "usuario") setFeedbacks(data.filter(fb => fb.usuario_id === id));
        else setFeedbacks(data.filter(fb => fb.empresa_id === empresaId));
      });
  }, [id, tipo, empresaId]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const bodyToSend = { nome: formData.nome, email: formData.email };
    if (formData.senha.trim() !== "") bodyToSend.senha = formData.senha;

    let response;
    if (tipo === "usuario") {
      response = await fetch(`${API_URL}/usuarios/${id}`, {
        method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(bodyToSend)
      });
    } else if (tipo === "empresa") {
      response = await fetch(`${API_URL}/empresas/${empresaId}`, {
        method: "PUT", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          nome: bodyToSend.nome, email: bodyToSend.email, senha: bodyToSend.senha,
          cnpj: formData.cnpj, setor: formData.setor, descricao: formData.descricao
        })
      });
    }

    if (!response.ok) { setMensagem("Erro ao atualizar"); return; }

    const updated = await response.json();
    salvarUsuario({ id: updated.id, nome: updated.nome, email: updated.email, tipo, empresa_id: empresaId });
    setMensagem("Dados atualizados"); setEditMode(false);
          ...
          <div className="Perfil-Info">
            {!editMode ? (
              <>
                ...
                <button type="button" className="Edit_Profile_Button" onClick={() => setEditMode(true)}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button type="button" className="Edit_Profile_Button" onClick={() => setEditMode(false)}>Cancelar</button>
              </form>
            )}
          </div>
        </div>
        ...
        <Table className="Avaliacao-Pesona">
          <TableHeader>
            <TableRow>
              ...
              {tipo === "usuario" && <TableHead className="Titulo-Tabela">Empresa</TableHead>}
              {tipo === "empresa" && <TableHead className="Titulo-Tabela">Sentimento</TableHead>}
              <TableHead className="Titulo-Tabela">Data</TableHead>
          ...
          <TableBody>
            {feedbacks.map(fb => {
              const confidencia = (fb.conf_sentimento * 100).toFixed(1);
              const acimaLimite = confidencia > conf_min;

              return (
                <TableRow key={fb.id}>
                  <TableCell>
                    <Link to={`/feedback/${fb.id}`} style={{ textDecoration: "none", color: "#0070f3" }}>
                      {fb.titulo}
                    </Link>
                  </TableCell>

                  {tipo === "usuario" && <TableCell>{fb.empresa_nome || "Desconhecido"}</TableCell>}

                  {tipo === "empresa" && (
                    <TableCell style={{ color: acimaLimite ? corNota[fb.nota_sentimento] : "#777" }}>
                      {acimaLimite ? mapaNotas[fb.nota_sentimento] : "Indeterminado"}
                    </TableCell>
                  )}
                  ...
                  <TableCell style={{ color:
                    fb.status === "Aberto" ? "#C50000" :
                    fb.status === "Respondida" ? "#00B327" : "#666"
                  }}>{fb.status}</TableCell>
                </TableRow>
              );
            })}
